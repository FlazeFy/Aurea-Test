    ...
    describe("Failed cases", () => {
        ...
        it("should fail with failed validation : payload's character length is more than maximum requirement", () => {
            cy.request({
                ...
                cy.expect(res.body.data.allergic_desc).to.eq("allergic_desc must be at most 255 characters")
            })
        })
        it("should fail with failed validation : required payload's is empty", () => {
            cy.request({
                ...
                cy.expect(res.body.data.allergic_context).to.eq("allergic_context is required")
            })
        })
